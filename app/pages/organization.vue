...
const filteredRows = computed(() => {
  ...
  return people.filter((person) => {
    return Object.values(person).some((value)   => {
      ...
      return String(value).toLowerCase().replace(/\s/g, '').includes(q.value.toLowerCase().replace(/\s/g, ''))
    })
  })
})
...
<template>
    <UDashboardPage>
        <UDashboardPanel grow>
            <UDashboardNavbar title="Organization Management">
                <template #right>
          <UTooltip
            text="Notifications"
            :shortcuts="['N']"
          ...
          <UDropdown :items="itemsnavbar">
            ...
            <UDashboardPanelContent>
                <div class="flex justify-between px-3 py-3.5 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <UButton color="primary" @click="openPopup"> 
                      เพิ่มข้อมูลองค์กร
                    </UButton>
                </div>
                <UTable :rows="paginatedFilteredRows" :columns="columns">
